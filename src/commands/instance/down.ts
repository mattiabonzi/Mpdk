...
export default class InstanceDown extends BaseCommand<typeof InstanceDown> {
  ...
  public async run(): Promise<void> {
    ...
    if (!this.opt.force) {
      await inquirer.prompt({
        type: 'confirm',
        name: 'continue',
        message: `ARE YOU REALLY SURE you want to take down ${this.opt.all?'all instance?':this.mpdk.instance.name}?`
      });
    }
    try {
      ui.action.start('Taking down');
      if (this.opt.all) {
        var promises:Promise<void>[] = [];
        Object.values(this.mpdk.getConfFromAllInstance()).forEach(conf => {
          var insta = (new Instance(this.mpdk)).load(conf);
          promises.push(insta.down());
        });
      ...
      this.success("Instance has been taken down");
    } catch (e) {
      ui.throw(e);
    }
